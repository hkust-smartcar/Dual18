...
bool IsOneLine(uint8_t frontL, uint8_t frontR, uint8_t midL, uint8_t midR){
	if (frontL+frontR-midL-midR <15){
		return true;
	}
	return false;
}

bool IsTwoLine(uint8_t frontL, uint8_t frontR, uint8_t midL, uint8_t midR){
	if (frontL+frontR-midL-midR>20){
		return true;
	}
	return false;
}
...
int main() {
    ...
    PID servoPIDStraight(2000,0.03);
...
//    PID motorRPID(0.145,0.0,1.35, &REncoder,true);
	...
	float frontLinear, midLinear, diffLinear, pastLinear;
	float raw_frontLinear, raw_midLinear;
	float multiplier = 0.0, top_multiplier = 0.0;
	float front_left = 1, front_right = 1, mid_left = 1, mid_right = 1, back_left = 1, back_right = 1, top_left = 1, top_right = 1 , past_left = 1, past_right =1;
    	...
    	if(System::Time() != lastTime){
    		...
    		if (lastTime % cycle == 0 && filterCounter != 0){
    			...
    			if (multiplier != 0){
					...
					past_left = midL.front()*multiplier;
					past_right = midR.front()*multiplier;
    			}
				filterSum0 = 0;
				filterSum1 = 0;
				filterSum2 = 0;
				...
				//nearLoop
				else if(state == normal && IsTwoLine(raw_front_left,raw_front_right,midL.front(),midR.front()) && abs(top_left-top_right)>5){
					state = nearLoop;
					speed = loopSpeed;
					stateTime = lastTime;
				...
				else if((state == nearLoop || state == outLoop)  && IsOneLine(raw_front_left,raw_front_right,raw_mid_left,raw_mid_right) ){
					state = straight1;
					stateTime = lastTime;
				}
				else if(state == straight1 && abs(top_left-top_right)>15){
					state = turning;
					stateTime = lastTime;
				}
				else if(state == turning && IsTwoLine(raw_front_left,raw_front_right,midL.front(),midR.front()) && abs(top_left-top_right)>15){
					state = outLoop;
					stateTime = lastTime;
				}else if (state == outLoop && IsOneLine(raw_front_left,raw_front_right,raw_mid_left,raw_mid_right)){
					state = normal;
					stateTime = lastTime;
				}
...
//				}
				...
				pastLinear = 1/(float)past_left-1/(float)past_right;
...
//					}
					angle = servoPIDStraight.getPID(0.0,pastLinear);
				}
				else if (state == straight1 ){
					if(leftLoop){
						frontLinear = 1/(front_left-noise)-1/(front_right-(past_right-noise));
					}else{
						frontLinear = 1/(front_left-(past_left-noise))-1/(front_right-noise);
					}
						angle = servoPIDCurve.getPID(0.0,frontLinear);
					...
					angle = servoPIDCurve.getPID(0.0,pastLinear);
					led3.SetEnable(!0);
					led2.SetEnable(!1);
					led1.SetEnable(!1);
				}
