...
//	LcdTypewriter writer(GetWriterConfig(mylcdP));
	for (int y=top_line+2; y<top_line+3; y++){
		for (int x=1 ; x<40; x++){
			for(int j=0; j<3; j++){
				for(int i=top_line; i<top_line+3 ; i++){
					if(ret_cam_bit(j, i, camBuffer)==1){
						is_both_edge = true;
						break;
					}
			...
			if((is_x_edge==true)&&(is_y_edge==true)){
				...
				if((left_count<300)&&(left_count>280)){
...
//                test ++;
//                if(test<40){
//                		Servo.SetDegree((90+test)*10);
//                }
//                else{
//                		Servo.SetDegree((90+40)*10);
//                		test = -40;
//                }
...
//                char c[15];
                ...
                for (int j = 45; j < 50; j++) {
                    ...
                    if (r_counter < 4)
                        r_small++;
                    lcdP->SetRegion(Lcd::Rect(39-l_counter, j, 3,1));
                    lcdP->FillColor(Lcd::kPurple);
                    lcdP->SetRegion(Lcd::Rect(39+r_counter, j, 3,1));
...
//						left_corner = false;
                ...
                if(see_the_right_curve || see_the_left_curve){ //curve
					Motor.SetPower(15);
					Servo.SetDegree(900 - (1 * (-temp) + 0 * derivative) * 10);
					led2.Switch();
                	}
                else if(left_corner && right_corner){ //corner
					Servo.SetDegree(900 - (1.35 * (-temp) + 0.25 * derivative) * 10);
					Motor.SetPower(40);
				}
				else if(Lstate == loop_state::start && (Tstate == left_loop || Tstate == right_loop)){ //start of loop
					Servo.SetDegree(900 - (1.45 * (-temp) + 0 * derivative) * 10);
					Motor.SetPower(25);
				}
				else if (((l_large && r_large) || l_small || r_small) && Tstate!=magnetic){ //if no line visible on both side
					if(temp<20 && temp>-20)
						Servo.SetDegree(900 - (0.82 * (-temp) + 0.15* derivative) * 10);
					else if(temp<45 && temp>-45)
						Servo.SetDegree(900 - (1.4 * (-temp) + 0 * derivative) * 10);
					else
						Servo.SetDegree(900 - (1.5 * (-temp) + 0 * derivative) * 10);
					if(Tstate == magnetic)
						Motor.SetPower(140);
					else
						Motor.SetPower(105);
                    ...
                    Servo.SetDegree(900 - (0.84 * (-temp) + 0.22 * derivative) * 10);
                    if((l_large || r_large) && temp>15 && temp<-15){
                    		if(Tstate == magnetic)
                    			Motor.SetPower(130);
                    		else
                    			Motor.SetPower(105);
                    ...
                    if(Tstate == magnetic)
                    		Motor.SetPower(150);
                    else if(l_large || r_large)
                    	    Motor.SetPower(145);
                    	else
                    		Motor.SetPower(140);
                }
                else{
					Servo.SetDegree(900 - (1.52 * (-temp) + 0 * derivative) * 10);
					if(Tstate == magnetic)
						Motor.SetPower(145);
					else if((l_large || r_large) && Tstate!=right_loop && Tstate!=left_loop)
					    	Motor.SetPower(115);
					else
						Motor.SetPower(140);
                }
...
//				sprintf(c,"l :%d   ",l_large);
//				writer.WriteBuffer(c,15);
//	            lcdP->SetRegion(Lcd::Rect(0,100,120,15));
//	            sprintf(c,"r :%d   ",r_large);
//				writer.WriteBuffer(c,15);
